...
contract("FragmentTemplate", accounts => {
  ...
  it("should rez a entity from template", async () => {
    const contract = await nft.deployed();
    const tx = await contract.rez(tokenTwo, "Token Two", "TWO", { from: accounts[1] });
    fragmentOneEntity = tx.logs[1].args.newContract;
  ...
  it("should not rez a entity from template", async () => {
    ...
    try {
      await contract.rez(tokenTwo, "Token Two", "TWO", { from: accounts[1] });
    } catch (e) {
  ...
  it("should mint an from entity as buyer", async () => {
    const contract = await nft.deployed();
    const entity = new web3.eth.Contract(entityNft.abi, fragmentOneEntity);
    const empty = new Uint8Array(1024);
    const parts = [
    ...
    const tx = await entity.methods
      ...
      .send({ from: accounts[4], gas: 500000, value: web3.utils.toWei("0.1", "ether") });
